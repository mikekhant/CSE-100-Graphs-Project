 ...
 */ 
 ...
 class tupleComp {
     public:
        bool operator()(tuple<string, string, double> lhs, tuple<string, string, double> rhs) {
 ...
 class Node {
    public:         
        ...
        /**
         * Constructor, which initializes an Node object
         */
        Node(string symbol) {
            this->symbol = symbol;
        }
...
class dNode {
    public:
        ...
        bool done;
        string symbol;
        vector<pair<string,double>> dedges;

        dNode(double distance, dNode* previous, bool done, string symbol, vector<pair<string,double>> dedges) {
